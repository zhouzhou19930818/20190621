...
export default class Program extends Component {
    ...
    componentDidMount() {
        ...
        this.getProgramHistoryDetail({ cluster: this.context.clusterValue });
        this.getOverRunningTimeProgram(this.context.clusterValue, overRunningTime);
    }
    ...
    componentWillReceiveProps(nextProps, nextContext) {
        if (nextContext.clusterRequestError) { // 请求集群有误
            ...
            this.getMemoryCpuTrend({ cluster: nextContext.clusterValue }); // 获取内存趋势图数据并加载图表
            this.getProgramHistoryDetail({ cluster: nextContext.clusterValue });
    ...
    getMemoryCpuTrend = ({ cluster, startTime, endTime }) => {
        ...
        api.getProgramCpuAndMemoryTrend({
            ...
            .then(res => {
                ...
                this.memoryCpuData = data.pointDTOS;

                const startPosition = 75, endPosition = 100;
                const startIndex = Math.floor(startPosition / 100 * this.memoryCpuData.length - 1);
                const endIndex = Math.floor(endPosition / 100 * this.memoryCpuData.length - 1);
                const startTime = this.memoryCpuData[startIndex].time;
                const endTime = this.memoryCpuData[endIndex].time;

                let x = [], maxY = data.rightYMax,
                    memoryY1 = [], cpuY1 = [], y2 = [], y3 = [], y4 = [];
                const split = Math.round(this.memoryCpuData.length / 3);
                const interval = data.interval;
                this.memoryCpuData && this.memoryCpuData.forEach((d, i) => {
                    x.push(getXAxisData(d.time, i, interval, this.memoryCpuData.length, split, x));
                    memoryY1.push(d.memoryRate * 100);
                    cpuY1.push(d.cpuRate * 100);
    ...
    refresh = () => {
        this.updateState({
            ...
            this.getMemoryCpuTrend({ startTime: this.totalStartTime, endTime: this.totalEndTime });
        });
    };
    ...
    onHistoryTimeChange = (e) => {
        ...
        this.setState({ historyTimeType: type }, () => {
                ...
                this.getProgramHistoryDetail({ startTime: this.historyStartTime, endTime: this.historyEndTime });
            }
        );
    };
    ...
    onTotalDatePickerChange = (attr) => (obj, time) => {
        ...
        if (this.customTotalStartTime && this.customTotalEndTime) {
            ...
            this.getMemoryCpuTrend({ startTime: this.customTotalStartTime, endTime: this.customTotalEndTime });
        }
    };
    ...
    trendSliderEvent = (start, end) => {
        const len = this.memoryCpuData.length;
        if (!len) return;

        const startIndex = Math.floor(start * len - 1);
        const endIndex = Math.floor(end * len - 1);
        const startTime = this.memoryCpuData[startIndex < 0 ? 0 : startIndex].time;
        const endTime = this.memoryCpuData[endIndex > len - 1 ? len - 1 : endIndex].time;
    ...
    onHistoryTableChange = (pagination) => {
        if (pagination && pagination.current) {
            this.updateState({
                ...
                this.getProgramHistoryDetail({ currentPage: pagination.current });
            });
        }
    };
    ...
    onHistorySearchChange = (value) => {
        ...
        const isDifferent = trimValue !== this.state.historySearchValue.trim();
        this.updateState({
            historyTable: {
                loading: { $set: isDifferent },
                pagination: {
                    current: { $set: 1 }
                },
            },
            ...
            this.getProgramHistoryDetail({ programName: trimValue, currentPage: 1 });
        });
    };
