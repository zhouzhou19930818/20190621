...
import { Tabs, message, Input, Steps, Badge } from 'antd';
...
class ListManage extends Component {
    ...
    //步骤点击事件
    onChange = current => {
        if (this.props.latestStep !== undefined && current > this.props.latestStep) {
            return;
        }
    ...
    mouseEvent = () => {
        ...
        const end = (e) => {
            ...
            if (isDown) {
                isDown = false;
                offsetY = 0;
                this.props.changeIsExpendContent1(false);
            }
        };
        // 鼠标按下方块
...
//根据任务id获取任务目前处于哪个步骤
    ...
    getStepStatus = (index) => {
        if (!this.props.stepStatus || !this.props.stepStatus[index]) return 'wait';
                    ...
                    <div className="drag-container disk-drag-container">
                        ...
                        <div className="drag-content disk-drag-content" id="disk-dragContent">
                            ...
                            <div>
                                ...
                                <Steps current={current} progressDot={customDot} onChange={props.taskId ? this.onChange : null}>
                                    {
                                        this.steps.map((item, index) => {
                                            return (
                                                <Step
                                                    title={(
                                                            <Badge
                                                                count={props.stepStatus && props.stepStatus[index] ? props.stepStatus[index].fail : 0}
                                                    ...
                                                    className={props.taskId && props.latestStep !== undefined && props.latestStep >= index ? 'step-enabled' : 'step-disbled'}
                                                />
                                            );
                                        })
                                    }
