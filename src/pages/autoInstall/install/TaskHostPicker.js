...
export default class TaskHostPicker extends Component {
    ...
    getList = () => {
        ...
        api.getAllHost().then(res => {
            // console.log(res.data.data.map(d=>d.res.data.data.automaticHostId))
            if (res.data.success !== 'true') {
    ...
    onRowSelect = (selectedRowKeys,dataSource) => {
        const result = dataSource.map(d=>d.automaticHostId)
        const lastHostIds =result.join(',')
        console.log(result)
        this.setState({ selectedRowKeys }, () => {
            this.props.changeState({
                task: {
                    hostIds: { $set: lastHostIds }
                }
            })
        });
    };
